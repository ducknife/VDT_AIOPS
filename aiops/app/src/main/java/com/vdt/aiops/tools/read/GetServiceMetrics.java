...
public class GetServiceMetrics {
    ...
    @Tool(description = """
                ...
                - redis: isUp, evictedKeysPerSec (eviction = memory pressure / OOM), memUsedBytes, memMaxBytes, memUsedPercent
    ...
    public String fetchMetrics(String service) {
        ...
        String probeStr = metricOfService.getProbes().isEmpty()
                ...
                : metricOfService.getProbes().entrySet().stream()
                        .map(e -> e.getKey() + "="
                                + (e.getValue() == null ? "n/a" : String.format("%.3f", e.getValue())))
                        .collect(Collectors.joining(", "));

        return String.format(
                "Service: %s [%s]%n"
                        + "cpuPercent=%.1f%%, memPercent=%.1f%% (%.0f MB used)%n"
                        + "%s",
