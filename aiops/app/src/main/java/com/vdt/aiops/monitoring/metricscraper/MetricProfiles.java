...
public class MetricProfiles {
    ...
    private static final Map<String, List<MetricProbe>> BY_SERVICE = Map.of(
            "redis", List.of(
                    ...
                    new MetricProbe("keyspaceMissesPerSec", "rate(redis_keyspace_misses_total[1m])"), // cache-miss storm (hệ quả OOM)
                    new MetricProbe("memUsedBytes", "redis_memory_used_bytes"),
                    new MetricProbe("memMaxBytes", "redis_memory_max_bytes"),
                    new MetricProbe("memUsedPercent", "redis_memory_used_bytes / redis_memory_max_bytes * 100")),
            "postgres", List.of(
                    new MetricProbe("isUp", "pg_up"),
                    new MetricProbe("activeConnections", "sum(pg_stat_activity_count{state=\"active\"})"), // active vs total cho exhaustion
                    ...
                    new MetricProbe("connUtilPercent", "sum(pg_stat_activity_count) / scalar(pg_settings_max_connections) * 100")),
            "nginx", List.of(
