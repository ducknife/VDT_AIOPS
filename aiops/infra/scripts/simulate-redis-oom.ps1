...
# Nhet keys lon vao redis cho vuot 256MB. Hau qua: cache miss -> node-api query postgres -> latency tang.
# Keys KHONG co TTL -> trang thai BEN, khong tu hoi.
# Recover: .\recover.ps1 redis-oom   (xoa oom:flood:*)
param(
...
Write-Host ""
Write-Host "=== [SIM] Redis OOM (maxmemory exhaustion) ===" -ForegroundColor Yellow
...
# Fill server-side bang Lua (EVAL) -- tin cay tren Windows.
# KHONG dung "... | docker exec -i ... --pipe": PowerShell pipe stdin vao --pipe hong -> mem khong tang.
Write-Host ""
Write-Host "[fill] dang nhet du lieu vao Redis..." -ForegroundColor Red
$bytes = $ValueSizeKB * 1024
docker exec aiops-redis redis-cli EVAL `
...
# Cache miss storm: xoa cache key roi goi /api/users -> ep node-api query postgres
Write-Host ""
Write-Host "[impact] ep cache miss -- goi /api/users 5 lan:" -ForegroundColor Yellow
1..5 | ForEach-Object {
